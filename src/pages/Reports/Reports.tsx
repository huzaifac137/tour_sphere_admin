...
const Reports = () => {
  const adminToken = useSelector(
    (state: any) => state.auth.admin?.data?.accessToken,
  );
  const [reports, setreports] = useState([]);
  const allData = async () => {
    try {
      const res = await axios.get(`${serverLink}api/admin/allreport`, {
        headers: {
          token: `Bearer ${adminToken}`,
        },
      });
      setreports(res.data?.data?.allReports);
    } catch (error) {
      alert('Error Getting All Reports');
    }
  };
  useEffect(() => {
    allData();
  ...
  const handledelte = async (id: any) => {
    try {
      await axios.delete(`${serverLink}api/admin/deletereport/${id}`, {
        headers: { token: `Bearer ${adminToken}` },
      });
      const checked = reports.filter((lis: any) => lis._id !== id);
      setreports(checked);
  ...
  const reportDetailsPage = (id: any) => {
    navigate(`/report/${id}`);
  };
  ...
  const displayItems = reports?.slice(pageVisited, pageVisited + itemsPerPage);
  ...
  return (
    <>
      <Breadcrumb pageName="Music Goals" />
      <ToastContainer />
      ...
      <ReactPaginate
        ...
        activeLinkClassName={
          'page-link relative block py-1.5 px-3 border-0 bg-[#231f21] dark:bg-[#ffffff] dark:text-[#231f21] text-[#ffffff] outline-none transition-all duration-300  hover:bg-gray-600 shadow-md focus:shadow-md'
        }
        disabledClassName={
